		    ...
		    Context                *parent)
{
	...
	return gtk_source_highlighter_get_tag_for_style (ce->priv->highlighter, style, parent_tag);
}
					    ...
					    gboolean           synchronous)
{
	...
	{
		gtk_source_highlighter_ensure_highlight (ce->priv->highlighter, start, end);
	...
	else
	{
		if (gtk_text_iter_get_line (start) < invalid_line)
		{
			...
			gtk_source_highlighter_ensure_highlight (ce->priv->highlighter,
								  start,
								  &valid_end);
		}
					 ...
					 GtkTextBuffer   *buffer)
{
	...
	if (ce->priv->buffer != NULL)
	{
		...
		gtk_source_highlighter_attach_buffer (ce->priv->highlighter,
						       NULL, 
						       NULL);
					    ...
					    GtkSourceStyleScheme *scheme)
{
	...
	gtk_source_highlighter_set_style_scheme (ce->priv->highlighter, scheme);
}
...
_gtk_source_context_engine_new (GtkSourceContextData *ctx_data)
{
	...
	ce->priv->highlighter = gtk_source_highlighter_new (ce);
	gtk_source_highlighter_set_styles_map (ce->priv->highlighter, ENGINE_STYLES_MAP (ce));
	       ...
	       gint                    time)
{
	...
	while (TRUE)
	{
		...
		gtk_source_highlighter_invalidate_region (ce->priv->highlighter, 
							   &line_start, &line_end);
		analyzed_end = line_end_offset;
		invalid = get_invalid_segment (ce);
