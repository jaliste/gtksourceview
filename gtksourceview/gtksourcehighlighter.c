 ...
 *  Copyright (C) 2010, 2011 - José Aliste
 * 
...
G_DEFINE_TYPE (GtkSourceHighlighter, gtk_source_highlighter, G_TYPE_OBJECT)

static void set_tag_style_hash_cb (const char           *style,
				   GSList               *tags,
...
GtkTextTag *
gtk_source_highlighter_get_tag_for_style (GtkSourceHighlighter *highlighter,
				    	  const gchar          *style,
...
/**
 * gtk_source_highlighter_invalidate_region
 * 
 * @highlighter: 
 ...
 */ 
void
gtk_source_highlighter_invalidate_region (GtkSourceHighlighter *highlighter, 
					  const GtkTextIter    *start,
					  const GtkTextIter    *end)
{
...
/**
 * gtk_source_highlighter_ensure_highlight:
 *
 * @highlighter: a #GtkSourceHighlighter.
 * @start: the beginning of the region to highlight.
 * @end: the end of the region to highlight.
 ...
 */
void
gtk_source_highlighter_ensure_highlight (GtkSourceHighlighter *highlighter,
					 const GtkTextIter    *start,
					 const GtkTextIter    *end)
{
 ...
 */
void
gtk_source_highlighter_attach_buffer (GtkSourceHighlighter *highlighter,
				      GtkTextBuffer        *buffer,
				      Segment		   *root_segment)
{
	g_return_if_fail (GTK_SOURCE_IS_HIGHLIGHTER (highlighter));
	g_return_if_fail (GTK_IS_TEXT_BUFFER (buffer) || buffer == NULL);
 ...
 */
void
gtk_source_highlighter_set_style_scheme (GtkSourceHighlighter *highlighter,
...
void
gtk_source_highlighter_set_styles_map (GtkSourceHighlighter *highlighter,
				       GHashTable	    *styles)
{
	g_return_if_fail (GTK_SOURCE_IS_HIGHLIGHTER (highlighter));
...
static void
gtk_source_highlighter_dispose (GObject *object)
{
	...
	{
		g_object_unref (highlighter->priv->style_scheme);
		highlighter->priv->style_scheme = NULL;
	}

	gtk_source_highlighter_attach_buffer (highlighter, NULL, NULL);

	G_OBJECT_CLASS (gtk_source_highlighter_parent_class)->dispose (object);
}
...
gtk_source_highlighter_finalize (GObject *object)
{
	...
	G_OBJECT_CLASS (gtk_source_highlighter_parent_class)->finalize (object);
}

static void
gtk_source_highlighter_class_init (GtkSourceHighlighterClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->dispose = gtk_source_highlighter_dispose;
...
static void
gtk_source_highlighter_init (GtkSourceHighlighter *highlighter)
{
...
gtk_source_highlighter_new (GtkSourceContextEngine *ce)
{
	GtkSourceHighlighter *highlighter;
